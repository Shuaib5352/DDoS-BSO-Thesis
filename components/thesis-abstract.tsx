...
The BSO-Hybrid RF model achieved ${bso.accuracy}% accuracy, ${bso.precision}% precision, ${bso.recall}% recall, ${bso.f1Score}% weighted F1-score, ${bso.f1Macro}% macro F1-score, and ${bso.aucRoc}% AUC-ROC on the held-out test set (n=${DATASET_STATISTICS.totalFlows.testing.toLocaleString("en-US")}). Compared to 11 baseline models, the proposed framework outperformed SVM (${svm.accuracy}%), Decision Tree (${dt.accuracy}%), Naive Bayes, and Logistic Regression, while achieving competitive accuracy to XGBoost (${xgb.accuracy}%) with 51.3% fewer features. The Matthews Correlation Coefficient (MCC) of ${bso.mcc.toFixed(4)} confirms strong multi-class agreement. Stratified 10-fold cross-validation yielded ${cv.mean.accuracy.toFixed(2)}% ± ${cv.std.accuracy}% accuracy, demonstrating model stability and generalizability.
...
BSO-Hibrit RF modeli, ayrılmış test seti (n=${DATASET_STATISTICS.totalFlows.testing.toLocaleString("tr-TR")}) üzerinde %${bso.accuracy} doğruluk, %${bso.precision} kesinlik, %${bso.recall} duyarlılık, %${bso.f1Score} ağırlıklı F1-skor, %${bso.f1Macro} makro F1-skor ve %${bso.aucRoc} AUC-ROC değerleri elde etmiştir. 11 temel model ile karşılaştırıldığında, önerilen çerçeve SVM (%${svm.accuracy}), Karar Ağacı (%${dt.accuracy}) ve Naive Bayes gibi modellerden üstün performans gösterirken, XGBoost (%${xgb.accuracy}) marjinal olarak daha yüksek doğruluk elde etmiştir — ancak %51.3 daha fazla öznitelik gerektirir. ${bso.mcc.toFixed(4)} değerindeki Matthews Korelasyon Katsayısı (MCC), güçlü çok sınıflı uyumu doğrulamaktadır. Tabakalı 10-katlı çapraz doğrulama %${cv.mean.accuracy.toFixed(2)} ± %${cv.std.accuracy} doğruluk ile modelin kararlılığını ve genellenebilirliğini göstermiştir.
