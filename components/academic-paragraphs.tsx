            ...
            <Tabs defaultValue="setup" className="w-full">
                ...
                <TabsContent value="statistical" className="space-y-4 mt-4">
                    <ParagraphCard
                        ...
                        textEN={`Statistical significance was assessed using 10-fold stratified cross-validation with SMOTE applied inside each fold to prevent data leakage, paired t-tests, and Wilcoxon signed-rank tests at α = 0.05. The BSO-Hybrid RF achieved a mean cross-validation accuracy of ${CROSS_VALIDATION.mean.accuracy}% ± ${CROSS_VALIDATION.std.accuracy} (σ). Pairwise comparisons against all 11 benchmark models showed: BSO-Hybrid RF significantly outperformed ${STATISTICAL_TESTS.filter(t => t.significant && t.cohenD > 0).length} models (p < 0.05), performs comparably to Random Forest (${STATISTICAL_TESTS[4].improvement}, p = ${STATISTICAL_TESTS[4].pValue}) and GWO-RF (${STATISTICAL_TESTS[3].improvement}, p = ${STATISTICAL_TESTS[3].pValue}) while using only ${bso.featuresUsed} features (51.3% reduction). XGBoost achieved the highest CV F1-Macro (${STATISTICAL_TESTS[10].improvement}, p = ${STATISTICAL_TESTS[10].pValue}) using all ${xgb.featuresUsed} features. The BSO-Hybrid RF's primary contribution is competitive accuracy with 51.3% dimensionality reduction, making it suitable for resource-constrained IoT environments.`}
                        textTR={`İstatistiksel anlamlılık, veri sızıntısını önlemek için SMOTE'un her katlam içinde uygulandığı 10 katlı tabakalı çapraz doğrulama, eşleştirilmiş t-testleri ve Wilcoxon işaretli sıralama testleri (α = 0.05) kullanılarak değerlendirilmiştir. BSO-Hybrid RF, %${CROSS_VALIDATION.mean.accuracy} ± ${CROSS_VALIDATION.std.accuracy} (σ) ortalama çapraz doğrulama doğruluğu elde etmiştir. 11 model ile ikili karşılaştırmalar: BSO-Hybrid RF, ${STATISTICAL_TESTS.filter(t => t.significant && t.cohenD > 0).length} modele karşı anlamlı üstünlük (p < 0.05), Random Forest (${STATISTICAL_TESTS[4].improvement}, p = ${STATISTICAL_TESTS[4].pValue}) ve GWO-RF (${STATISTICAL_TESTS[3].improvement}, p = ${STATISTICAL_TESTS[3].pValue}) ile eşdeğer performans gösterir — yalnızca ${bso.featuresUsed} özellik kullanarak (%51.3 azaltma). XGBoost en yüksek CV F1-Macro değerine ulaşmıştır (${STATISTICAL_TESTS[10].improvement}, p = ${STATISTICAL_TESTS[10].pValue}) ancak tüm ${xgb.featuresUsed} özellik gerektirir. BSO-Hybrid RF'nin temel katkısı %51.3 boyut azaltma ile rekabetçi doğruluktur.`}
                        textAR={`تم تقييم الدلالة الإحصائية باستخدام التحقق المتقاطع الطبقي 10 طيات مع تطبيق SMOTE داخل كل طية لمنع تسريب البيانات، واختبارات t المزدوجة، واختبارات ويلكوكسون عند α = 0.05. حقق BSO-Hybrid RF متوسط دقة ${CROSS_VALIDATION.mean.accuracy}% ± ${CROSS_VALIDATION.std.accuracy}. أظهرت المقارنات مع 11 نموذجاً: BSO-Hybrid RF تفوق على ${STATISTICAL_TESTS.filter(t => t.significant && t.cohenD > 0).length} نماذج (p < 0.05)، وأداء مكافئ لـ Random Forest (${STATISTICAL_TESTS[4].improvement}، p = ${STATISTICAL_TESTS[4].pValue}) وGWO-RF (${STATISTICAL_TESTS[3].improvement}، p = ${STATISTICAL_TESTS[3].pValue}) باستخدام ${bso.featuresUsed} ميزة فقط. XGBoost حقق أعلى CV F1-Macro (${STATISTICAL_TESTS[10].improvement}، p = ${STATISTICAL_TESTS[10].pValue}) باستخدام جميع الـ ${xgb.featuresUsed} ميزة. مساهمة BSO-Hybrid RF هي دقة تنافسية مع تقليل الأبعاد بنسبة 51.3%.`}
                    />
                </TabsContent>
