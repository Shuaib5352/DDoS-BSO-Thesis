            ...
            <Tabs defaultValue="setup" className="w-full">
                ...
                <TabsContent value="statistical" className="space-y-4 mt-4">
                    <ParagraphCard
                        ...
                        textEN={`Statistical significance was assessed using 10-fold stratified cross-validation, paired t-tests, and Wilcoxon signed-rank tests at α = 0.05. The BSO-Hybrid RF achieved a mean cross-validation accuracy of ${CROSS_VALIDATION.mean.accuracy}% ± ${CROSS_VALIDATION.std.accuracy} (σ), demonstrating high stability with no evidence of overfitting. Paired comparisons revealed: BSO-Hybrid RF significantly outperformed Decision Tree (+${STATISTICAL_TESTS[1].improvement}, p < 0.001, Cohen's d = ${STATISTICAL_TESTS[1].cohenD}), Logistic Regression (+${STATISTICAL_TESTS[2].improvement}, p < 0.001), BSO-SVM (+${STATISTICAL_TESTS[7].improvement}, p < 0.001), Naive Bayes (+${STATISTICAL_TESTS[9].improvement}, p < 0.001), KNN (+${STATISTICAL_TESTS[8].improvement}, p < 0.001), and PSO-RF (+${STATISTICAL_TESTS[4].improvement}, p < 0.001). BSO-Hybrid RF was statistically comparable to GWO-RF (${STATISTICAL_TESTS[6].improvement}, p = ${STATISTICAL_TESTS[6].pValue}, not significant) but uses ${gwoRf.featuresUsed - bso.featuresUsed} fewer features. XGBoost showed marginal superiority (${STATISTICAL_TESTS[3].improvement}, p = ${STATISTICAL_TESTS[3].pValue}) but requires all ${xgb.featuresUsed} features.`}
                        textTR={`İstatistiksel anlamlılık, 10 katlı tabakalı çapraz doğrulama, eşleştirilmiş t-testleri ve Wilcoxon işaretli sıralama testleri (α = 0.05) kullanılarak değerlendirilmiştir. BSO-Hybrid RF, %${CROSS_VALIDATION.mean.accuracy} ± ${CROSS_VALIDATION.std.accuracy} (σ) ortalama çapraz doğrulama doğruluğu elde etmiş olup aşırı uyum kanıtı olmaksızın yüksek kararlılık gösterir. Eşleştirilmiş karşılaştırmalar: BSO-Hybrid RF, Decision Tree'yi (+${STATISTICAL_TESTS[1].improvement}, p < 0.001, Cohen d = ${STATISTICAL_TESTS[1].cohenD}), Logistic Regression'ı (+${STATISTICAL_TESTS[2].improvement}), BSO-SVM'yi (+${STATISTICAL_TESTS[7].improvement}), Naive Bayes'i (+${STATISTICAL_TESTS[9].improvement}), KNN'yi (+${STATISTICAL_TESTS[8].improvement}) ve PSO-RF'yi (+${STATISTICAL_TESTS[4].improvement}) anlamlı şekilde geride bırakmıştır. GWO-RF ile istatistiksel olarak karşılaştırılabilirdir (${STATISTICAL_TESTS[6].improvement}, p = ${STATISTICAL_TESTS[6].pValue}, anlamlı değil) ancak ${gwoRf.featuresUsed - bso.featuresUsed} daha az özellik kullanır. XGBoost marjinal üstünlük (${STATISTICAL_TESTS[3].improvement}, p = ${STATISTICAL_TESTS[3].pValue}) gösterir ancak tüm ${xgb.featuresUsed} özelliği gerektirir.`}
                        textAR={`تم تقييم الدلالة الإحصائية باستخدام التحقق المتقاطع الطبقي 10 طيات، واختبارات t المزدوجة، واختبارات ويلكوكسون عند α = 0.05. حقق BSO-Hybrid RF متوسط دقة ${CROSS_VALIDATION.mean.accuracy}% ± ${CROSS_VALIDATION.std.accuracy}، مما يدل على استقرار عالٍ دون إفراط في التعلم. أظهرت المقارنات المزدوجة: تفوق BSO-Hybrid RF بشكل ملحوظ على Decision Tree (+${STATISTICAL_TESTS[1].improvement}، p < 0.001)، Logistic Regression (+${STATISTICAL_TESTS[2].improvement})، BSO-SVM (+${STATISTICAL_TESTS[7].improvement})، Naive Bayes (+${STATISTICAL_TESTS[9].improvement})، KNN (+${STATISTICAL_TESTS[8].improvement})، وPSO-RF (+${STATISTICAL_TESTS[4].improvement}). كان مماثلاً إحصائياً لـ GWO-RF (${STATISTICAL_TESTS[6].improvement}، p = ${STATISTICAL_TESTS[6].pValue}) لكنه يستخدم ${gwoRf.featuresUsed - bso.featuresUsed} ميزات أقل. أظهر XGBoost تفوقاً هامشياً (${STATISTICAL_TESTS[3].improvement}، p = ${STATISTICAL_TESTS[3].pValue}) لكنه يتطلب جميع الميزات الـ ${xgb.featuresUsed}.`}
                    />
                </TabsContent>
